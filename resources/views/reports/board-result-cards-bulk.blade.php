{{--
    Bulk board-result cards — every student's result card in one PDF,
    one per page. Reuses the same card layout as board-result-card.blade
    (single-student view) but wraps each in a page-break container so
    DomPDF renders them as separate pages.

    Kept as a partial-loop rather than @include so we don't pay the
    per-include overhead for a 40+ student class.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Board Results — {{ $exam->title }} — All Cards</title>
<style>
