...
<head>
<meta charset="UTF-8">
<title>Substitution Duty Slip — {{ $date->format('d M Y') }}</title>
<style>
...
<div class="hdr">
    <div class="sch-tag">Daily Substitution Duty Slip</div>
    <div class="sch-name">Teacher Cover Roster</div>
    <div class="date-line">{{ $date->format('l, d F Y') }}</div>
</div>

<div class="title-bar">Today's Cover Assignments</div>

<div class="summary">
    <strong>{{ $totalAssignments }}</strong> period(s) being covered by
    <strong>{{ $bySub->count() }}</strong> substitute teacher(s) today.
</div>

@if($bySub->isEmpty())
    <div class="empty">No substitution assignments for this date.<br>
