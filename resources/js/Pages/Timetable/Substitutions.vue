...
function changeDate(newDate) {
    router.get(route('timetable.substitutions'), { date: newDate }, { preserveScroll: true })
}
...
function submitAbsence() {
    ...
    if (!m) return
    router.post(route('timetable.substitutions.absence'), {
...
function generate() {
    router.post(route('timetable.substitutions.generate'), { date: props.date }, { preserveScroll: true })
}

function confirmA(a) {
    router.post(route('timetable.substitutions.confirm', a.id), {}, { preserveScroll: true })
}
function declineA(a) {
...
function submitReassign() {
    ...
    router.post(route('timetable.substitutions.reassign', reassignTarget.value.id),
...
<template>
    ...
    <AppLayout :breadcrumbs="[
        ...
        <div class="space-y-5 max-w-[1500px] mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-base-content/55 mt-1">
                        Mark absences (full day or partial) and auto-fill empty periods fairly.
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    <input v-model="dateInput" @change="changeDate(dateInput)" type="date"
                        class="input input-bordered input-sm rounded-xl text-sm font-mono" />
                    <a :href="route('timetable.substitutions.slip', { date })" target="_blank"
            ...
            <!-- HOW IT WORKS -->
            <div class="rounded-2xl border-2 border-amber-500/30 bg-amber-500/5 p-4">
                <div class="flex items-start gap-3">
                    <InformationCircleIcon class="w-5 h-5 text-amber-600 dark:text-amber-400 mt-0.5 shrink-0" />
                    <div class="text-sm text-base-content/75">
                        <p class="font-bold mb-1">How to handle today's absences</p>
                        <ol class="list-decimal list-inside text-xs space-y-0.5 text-base-content/65">
                            <li>Click <strong>Mark absent</strong> on a teacher's card. Choose <strong>Full day</strong> or <strong>From a specific period</strong> (for "left after lunch" cases).</li>
                            <li>Click <strong>Generate substitutions</strong> — the algorithm picks free, balanced cover teachers.</li>
                            <li>Review the table. Confirm, reassign, or decline each cover. Print the slip when done.</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div class="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-bold text-base-content/55">Absent today</p>
                        <p class="text-2xl font-extrabold tabular-nums leading-tight">{{ absentCount }}</p>
                    </div>
                </div>
                <div class="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-orange-500/15 text-orange-600 dark:text-orange-400 flex items-center justify-center shrink-0">
                        <ClockIcon class="w-5 h-5" />
                    </div>
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-bold text-base-content/55">Partial-day</p>
                        <p class="text-2xl font-extrabold tabular-nums leading-tight">{{ absentPartial }}</p>
                    </div>
                </div>
                <div class="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-bold text-base-content/55">Suggested</p>
                        <p class="text-2xl font-extrabold tabular-nums leading-tight">{{ stats.suggested }}</p>
                    </div>
                </div>
                <div class="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-bold text-base-content/55">Confirmed</p>
                        <p class="text-2xl font-extrabold tabular-nums leading-tight">{{ stats.confirmed }}</p>
                    </div>
                </div>
                <div class="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-sky-500/15 text-sky-600 dark:text-sky-400 flex items-center justify-center shrink-0">
                        <ArrowsRightLeftIcon class="w-5 h-5" />
                    </div>
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-bold text-base-content/55">Total covers</p>
                        <p class="text-2xl font-extrabold tabular-nums leading-tight">{{ stats.total }}</p>
                    </div>
                </div>
            </div>
            ...
            <section class="rounded-2xl border border-base-300 bg-base-100 overflow-hidden">
                <header class="px-5 py-3 border-b border-base-300 flex items-center gap-2 flex-wrap">
                    ...
                    <button @click="generate" class="ml-auto btn btn-primary btn-sm rounded-xl gap-1.5">
                        <BoltIcon class="w-4 h-4" /> Generate substitutions
                    </button>
                </header>
            ...
            <section v-if="assignments.length" class="rounded-2xl border border-base-300 bg-base-100 overflow-hidden">
                <header class="px-5 py-3 border-b border-base-300 flex items-center gap-2">
                    ...
                    <h2 class="text-sm font-bold">Step 2 — Review &amp; confirm covers</h2>
                </header>
            ...
            <div v-if="reassignTarget" class="modal modal-open">
                <div class="modal-box max-w-md">
                    <h3 class="text-base font-bold mb-3">Reassign substitute</h3>
