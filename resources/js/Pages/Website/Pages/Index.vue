...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Website' }, { label: 'Pages Content' }]">
        <div class="max-w-5xl mx-auto space-y-6">
            <div>
                <h1 class="text-2xl font-extrabold tracking-tight">Pages Content</h1>
                <p class="text-sm text-base-content/55 mt-0.5">
                    Manage the hero banner and content blocks shown on each public page · {{ pages.length }} page{{ pages.length === 1 ? '' : 's' }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <Link v-for="p in pages" :key="p.key"
                      ...
                      class="group surface surface-hover !p-5 block">
                    <div class="flex items-start justify-between gap-3">
                        <div class="w-11 h-11 rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center group-hover:bg-emerald-600 group-hover:text-white transition-all">
                            <Squares2X2Icon class="w-5 h-5" />
                        </div>
                        <a :href="p.route" target="_blank" rel="noopener"
                            @click.stop
                            class="btn btn-ghost btn-xs gap-1 text-base-content/55 hover:text-emerald-600">
                    ...
                    <div class="mt-4 space-y-1.5">
                        ...
                        <div v-if="p.auto_content" class="flex items-start gap-2 text-[11px] text-base-content/55">
                            <span class="w-3.5 h-3.5 rounded-full bg-sky-500/15 text-sky-600 dark:text-sky-400 flex items-center justify-center text-[8px] font-bold shrink-0 mt-0.5">A</span>
                            <span>Plus auto: {{ p.auto_content }}</span>
                        </div>
                    </div>

                    <div class="mt-4 pt-3 border-t border-base-200 flex items-center justify-end">
                        <span class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 inline-flex items-center gap-1 group-hover:gap-2 transition-all">
            ...
            <div class="surface !p-5 flex flex-row items-start gap-4">
                <DocumentTextIcon class="w-6 h-6 text-amber-500 shrink-0" />
                <div>
                    <h3 class="text-sm font-bold">How it works</h3>
                    <p class="text-xs text-base-content/65 mt-1.5 leading-relaxed">
