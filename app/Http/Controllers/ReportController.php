...
class ReportController extends Controller
{
    ...
    /**
     * Board-pattern primary term-wise result — the government format shared
     * by the school. One landscape page per class × section. Combines T-I,
     * T-II and T-III marks side by side for every subject plus a 10-mark
     * Co-Curricular column, with a summary block + fixed 10-tier grade
     * distribution at the bottom.
     *
     * Primary classes (ECD → 5) only — non-primary requests 404.
     *
     * ?format=xlsx → download the same layout as an Excel workbook.
     * Otherwise streams a PDF.
     *
     * READ-ONLY on the marks table. Never touches Mark rows. All numbers
    ...
    public function boardPrimaryResult(
        ...
        if ($request->query('format') === 'xlsx') {
            return \Maatwebsite\Excel\Facades\Excel::download(
                new \App\Exports\BoardPrimaryResultExport($payload),
                "{$slug}.xlsx"
            );
        }
